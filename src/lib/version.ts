...
export function resolvePackageVersion(importMetaUrl?: string): string {
  const injected =
    typeof process !== 'undefined' && typeof process.env.XCLI_VERSION === 'string'
      ? process.env.XCLI_VERSION.trim()
      : '';
...
export function resolveGitSha(importMetaUrl?: string): string | null {
  const injected =
    typeof process !== 'undefined' && typeof process.env.XCLI_GIT_SHA === 'string'
      ? process.env.XCLI_GIT_SHA.trim()
      : '';
  if (injected.length > 0) {
