...
function resolveFeaturesCachePath(): string {
  const override = process.env.XCLI_FEATURES_CACHE ?? process.env.XCLI_FEATURES_PATH;
  if (override && override.trim().length > 0) {
    return path.resolve(override.trim());
  }
  return path.join(homedir(), '.config', 'xcli', DEFAULT_CACHE_FILENAME);
}
...
function readOverridesFromEnv(): NormalizedFeatureOverrides | null {
  const raw = process.env.XCLI_FEATURES_JSON;
