...
const LIVE = process.env.XCLI_LIVE === '1';
...
const LIVE_NODE_ENV = (process.env.XCLI_LIVE_NODE_ENV ?? 'production').trim() || 'production';

function runBird(args: string[], options: { timeoutMs?: number } = {}): Promise<RunResult> {
  if (!LIVE) {
    throw new Error('runBird() called without XCLI_LIVE=1');
  }
...
d('live CLI (Twitter/X)', () => {
  const timeoutArg = (process.env.XCLI_LIVE_TIMEOUT_MS ?? '20000').trim();
  const cookieTimeoutArg = (process.env.XCLI_LIVE_COOKIE_TIMEOUT_MS ?? '30000').trim();
  ...
  beforeAll(async () => {
    ...
    const forcedTweetId = (process.env.XCLI_LIVE_TWEET_ID ?? '').trim();
    if (forcedTweetId) {
      if (!TWEET_ID_REGEX.test(forcedTweetId)) {
        throw new Error(`Invalid XCLI_LIVE_TWEET_ID (expected digits): "${forcedTweetId}"`);
      }
    ...
    const searchQuery = (
      process.env.XCLI_LIVE_SEARCH_QUERY ?? `from:${handle} -filter:replies -filter:retweets`
    ).trim();
    ...
    if (!TWEET_ID_REGEX.test(first)) {
      throw new Error(
        `Search returned no usable tweets. Query: "${searchQuery}". ` +
          `Override with XCLI_LIVE_SEARCH_QUERY.\n${search.stdout}`,
      );
    }
    tweetId = first;
  });
  ...
  it('follow/unfollow works (opt-in)', async () => {
    const followHandle = (process.env.XCLI_LIVE_FOLLOW_HANDLE ?? '').trim();
    if (!followHandle) {
      return;
    }
  ...
  it('bookmarks --folder-id works (opt-in)', async () => {
    const folderId = (process.env.XCLI_LIVE_BOOKMARK_FOLDER_ID ?? '').trim();
    if (!folderId) {
      return;
    }
    const bookmarks = await runBird(
  ...
  it('user-tweets returns JSON array', async () => {
    ...
    const testHandle = process.env.XCLI_LIVE_USER_TWEETS_HANDLE || 'X';
  ...
  it('user-tweets paged JSON returns { tweets, nextCursor }', async () => {
    const testHandle =
      process.env.XCLI_LIVE_USER_TWEETS_PAGED_HANDLE ?? process.env.XCLI_LIVE_USER_TWEETS_HANDLE ?? 'X';
    const userTweets = await runBird(
      [
        ...baseArgs,
        '--cookie-timeout',
  ...
  it('query-ids --fresh works (opt-in)', async () => {
    if (process.env.XCLI_LIVE_QUERY_IDS_FRESH !== '1') {
      return;
    }
  ...
  it('long-form tweet (article) extracts rich content (opt-in)', async () => {
    const longformTweetId = (process.env.XCLI_LIVE_LONGFORM_TWEET_ID ?? '').trim();
    if (!longformTweetId) {
  ...
  it('engagement mutations work (opt-in)', async () => {
    const engagementTweetId = (process.env.XCLI_LIVE_ENGAGEMENT_TWEET_ID ?? '').trim();
    if (!engagementTweetId) {
      return;
    }
    if (!TWEET_ID_REGEX.test(engagementTweetId)) {
      throw new Error(`Invalid XCLI_LIVE_ENGAGEMENT_TWEET_ID (expected digits): "${engagementTweetId}"`);
    }
