...
describe('cli-args', () => {
  ...
  it('rewrites bare tweet url to read command', () => {
    ...
    expect(result.argv).toEqual(['node', 'xcli', 'read', 'https://x.com/user/status/1234567890']);
  });

  it('rewrites bare tweet id to read command', () => {
    const result = resolveCliInvocation(['1234567890123456789'], known);
    expect(result.argv).toEqual(['node', 'xcli', 'read', '1234567890123456789']);
  });
  ...
  it('preserves leading options before inferred read command', () => {
    ...
    expect(result.argv).toEqual(['node', 'xcli', '--plain', 'read', 'https://x.com/user/status/1234567890']);
  });
