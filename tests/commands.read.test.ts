...
describe('replies command', () => {
  ...
  it('uses pagination when --max-pages is provided', async () => {
    ...
    try {
      await program.parseAsync(['node', 'xcli', 'replies', '123', '--max-pages', '2', '--json']);
  ...
  it('validates --max-pages is a positive integer', async () => {
    ...
    try {
      await expect(
        program.parseAsync(['node', 'xcli', 'replies', '123', '--all', '--max-pages', '-1']),
  ...
  it('validates --delay is a non-negative integer', async () => {
    ...
    try {
      await expect(program.parseAsync(['node', 'xcli', 'replies', '123', '--all', '--delay', '-100'])).rejects.toThrow(
        'exit 1',
      );
...
describe('thread command', () => {
  ...
  it('uses pagination when --max-pages is provided', async () => {
    ...
    try {
      await program.parseAsync(['node', 'xcli', 'thread', '123', '--max-pages', '2', '--json']);
      expect(pagedSpy).toHaveBeenCalledTimes(1);
      expect(unpagedSpy).toHaveBeenCalledTimes(0);
    } finally {
      pagedSpy.mockRestore();
  ...
  it('validates --max-pages is a positive integer', async () => {
    ...
    try {
      await expect(program.parseAsync(['node', 'xcli', 'thread', '123', '--all', '--max-pages', '0'])).rejects.toThrow(
        'exit 1',
      );
      expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid --max-pages'));
    } finally {
  ...
  it('validates --delay is a non-negative integer', async () => {
    ...
    try {
      await expect(program.parseAsync(['node', 'xcli', 'thread', '123', '--all', '--delay', 'abc'])).rejects.toThrow(
        'exit 1',
      );
      expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid --delay'));
    } finally {
