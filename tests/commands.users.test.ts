...
describe('users commands', () => {
  it('requires --all when --max-pages is provided', async () => {
    ...
    try {
      await expect(program.parseAsync(['node', 'xcli', 'following', '--max-pages', '2'])).rejects.toThrow('exit 1');
  ...
  it('rejects --max-pages when only --cursor is provided', async () => {
    ...
    try {
      await expect(
        program.parseAsync(['node', 'xcli', 'following', '--cursor', 'prev', '--max-pages', '2']),
  ...
  it('emits pagination JSON when --cursor is provided', async () => {
    ...
    await program.parseAsync(['node', 'xcli', 'following', '--cursor', 'prev', '--json']);
  ...
  it('prints next cursor in non-JSON single-page mode', async () => {
    ...
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    await program.parseAsync(['node', 'xcli', 'following', '--cursor', 'prev']);
  ...
  it('emits pagination JSON for --all results', async () => {
    ...
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    await program.parseAsync(['node', 'xcli', 'following', '--all', '--max-pages', '1', '--json']);
  ...
  it('prints next cursor when stopping at max pages in non-JSON mode', async () => {
    ...
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const run = program.parseAsync(['node', 'xcli', 'following', '--all', '--max-pages', '1']);
  ...
  it('deduplicates users across pages in --all mode', async () => {
    ...
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => undefined);
    const run = program.parseAsync(['node', 'xcli', 'following', '--all', '--json']);
    await vi.advanceTimersByTimeAsync(1000);
    await run;
